...
export default function Dashboard() {
  ...
  const filteredData = (selectedDayType === 'today' || selectedDayType === 'yesterday')
    ? todayData
    : hourlyData.filter(d => Number(d.route_id) === Number(selectedRoute) && d.day_type === selectedDayType);
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex space-x-4 mb-6">
        <button onClick={() => setTab('home')} className={`px-4 py-2 rounded ${tab === 'home' ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}>Home</button>
        <button onClick={() => setTab('correlation')} className={`px-4 py-2 rounded ${tab === 'correlation' ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}>Correlation</button>
        <button onClick={() => setTab('routes')} className={`px-4 py-2 rounded ${tab === 'routes' ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}>Routes</button>
      </div>
      ...
      {tab === 'correlation' && (
        <>
          ...
          {scatterData.length > 0 && (
            <>
              <h2 className="text-xl font-semibold text-center mb-2">Correlation between Delay Ratio and AQI</h2>
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart>
                  <CartesianGrid />
                  <XAxis type="number" dataKey="delay_ratio" name="Delay Ratio" label={{ value: 'Delay Ratio', position: 'insideBottom', offset: -5 }} />
                  <YAxis type="number" dataKey="aqi" name="AQI" label={{ value: 'AQI', angle: -90, position: 'insideLeft' }} />
                  ...
                  {regressionResult && (
                    <Line
                      ...
                      dot={false}
                      legendType="none"
                      data={scatterData.map(d => ({ delay_ratio: d.delay_ratio, aqi: regressionResult.predict(d.delay_ratio)[1] }))}
                    />
                  )}
                </ScatterChart>
              </ResponsiveContainer>
            ...
            <>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={filteredData}>
                  ...
                  <XAxis dataKey="hour" tickFormatter={h => `${h}:00`} label={{ value: 'Hour of Day', position: 'insideBottom', offset: -5 }} />
                  <YAxis label={{ value: selectedMetric.replaceAll('_', ' ').toUpperCase(), angle: -90, position: 'insideLeft' }} />
                  <Tooltip />
                  <Legend />
              ...
              {selectedRoute && (
                <div className="flex justify-center mt-6">
                  <img src={`/routes/Route_${selectedRoute}.jpg`} alt={`Route ${selectedRoute}`} className="max-w-full rounded-lg shadow-lg border border-gray-300" onError={(e) => { e.target.style.display = 'none'; }} />
                </div>
              )}
            </>
          )}
