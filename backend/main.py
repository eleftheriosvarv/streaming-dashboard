...
def get_latest_updates():
    try:
        ...
        return [
            TravelUpdate(**row) for row in rows
        ]
    ...
    try:
        ...
        cur.execute("""
            SELECT 
                ...
                ROUND(AVG(aqi)::numeric, 2) AS avg_aqi
            FROM travel_updates
            WHERE route_id = %s AND timestamp >= %s AND timestamp < %s
            GROUP BY route_id, day_type, hour
            ORDER BY hour;
...
def get_start_locations():
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT DISTINCT start_location FROM travel_updates WHERE start_location IS NOT NULL ORDER BY start_location;
        """)
        rows = cur.fetchall()
        conn.close()

        return {"start_locations": [row["start_location"] for row in rows]}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_correlation_data(start_location: str, type: str):
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT delay_ratio, aqi, timestamp FROM travel_updates WHERE start_location = %s ORDER BY timestamp;
        """, (start_location,))
        rows = cur.fetchall()
        conn.close()

        if not rows:
            return {"delay_ratio": [], "aqi": []}

        df = pd.DataFrame(rows, columns=["delay_ratio", "aqi", "timestamp"])
        ...
        if type == "same_minute":
            ...
            matched = merged[['delay_ratio', 'aqi']].dropna().to_dict('records')

        elif type in ["plus_one_hour", "plus_two_hours"]:
            minutes_offset = 60 if type == "plus_one_hour" else 120
            for idx, row in df.iterrows():
                target_min = row['timestamp'] + pd.Timedelta(minutes=minutes_offset - 20)
                target_max = row['timestamp'] + pd.Timedelta(minutes=minutes_offset + 20)
                potential = df[(df['timestamp'] >= target_min) & (df['timestamp'] <= target_max)]
                if not potential.empty:
                    matched.append({
                        "delay_ratio": row['delay_ratio'],
                        "aqi": potential.iloc[0]['aqi']
                    })
        else:
            raise HTTPException(status_code=400, detail="Invalid correlation type.")

        return {
            "delay_ratio": [m["delay_ratio"] for m in matched],
            "aqi": [m["aqi"] for m in matched]
        }
